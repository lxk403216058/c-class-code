...
typedef struct traininfor node1;

struct node
{
    struct traininfor data;
    struct node *next;
};
typedef  struct node *pemp;
...
void tickets (pemp trhead,int n)//读数据函数 
{
	...
	while (1==fread(&p->data,sizeof(struct traininfor),1,fp))
	{
		p->next=NULL;
		q->next=p;
		q=p;
		printf("车次号：%d",p->data.tnumber);
		p=(pemp)malloc(sizeof(node1));
	}
